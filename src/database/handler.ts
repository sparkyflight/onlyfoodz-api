...
import { OnlyfoodzPost, Token, User } from "./types.interface.js";
...
class Tokens extends Model implements Token {
	...
	static async createToken(creator_id: string): Promise<string | Error> {
		try {
	...
	static async getAllTokens(creatorid: string): Promise<Token[] | Error> {
		try {
...
const init = () => {
	sequelize.addModels([Users, OnlyfoodzPosts, Tokens]);
...
// Export the classes
export { Users, OnlyfoodzPosts, Tokens };
